...
type Props = {
  table: TableResponse | undefined;
  page: number;
  onPageChange: (page: number) => void;
  anomalyThreshold?: number | null;
};

export default function MortalityTable({ table, page, onPageChange, anomalyThreshold }: Props) {
  return (
        ...
        <tbody>
          {table?.data?.map((row) => (
            <tr key={`${row.facilityId}-${row.measureId}-${row.startDate}`} className="border-b">
              ...
              <td className={`p-2 ${row.mortalityRate !== null && anomalyThreshold != null && row.mortalityRate >= anomalyThreshold ? "anomaly-cell" : ""}`}>
                {row.mortalityRate ?? "-"}
              </td>
