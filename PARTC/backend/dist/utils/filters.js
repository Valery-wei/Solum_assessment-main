export function applyFilters(data, filters) {
    return data.filter((row) => {
        if (filters.year && row.year !== filters.year)
            return false;
        if (filters.month && row.month !== filters.month)
            return false;
        if (filters.state && row.state.toLowerCase() !== filters.state.toLowerCase())
            return false;
        if (filters.states?.length) {
            const set = new Set(filters.states.map((s) => s.toLowerCase()));
            if (!set.has(row.state.toLowerCase()))
                return false;
        }
        ...
        if (filters.facilityName) {
            ...
            if (!row.facilityName.toLowerCase().includes(keyword))
                return false;
        }
        return true;
    });
